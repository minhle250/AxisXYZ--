...
export interface SentinelContextType {
  ...
  setDistractionCount: (n: number) => void;
  focusIntensity: 'LOW' | 'MED' | 'HIGH';
  setFocusIntensity: (v: 'LOW' | 'MED' | 'HIGH') => void;
  currentTrack: string;
...
export const SentinelProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [focusIntensity, setFocusIntensity] = useState<'LOW' | 'MED' | 'HIGH'>('MED');
  const [currentTrack, setCurrentTrack] = useState('Axis Radio - LoFi');
  ...
  const contextValue = useMemo(() => ({
    ...
    resetFatigue
  }), [activeMode, fatigueLevel, notes, tasks, pomodoroTime, isTimerActive, isProcessing, isAboutOpen, currentBgIndex, isFocusMode, isMusicOpen, isMusicPlaying, isClockOpen, distractionCount, focusIntensity, currentTrack]);
